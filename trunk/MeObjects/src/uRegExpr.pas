...
ModifierX: the extended syntax. To extend your pattern's legibility by permitting whitespace and comments
 (?x) option: whitespace data characters in the pattern are totally ignored except when escaped or inside a character class. 
      And in addition, characters between an unescaped # outside a character class and the next newline character, inclusive, are also ignored.
      This can be changed within a pattern by a (?x) option setting. 
      this makes it possible to include comments inside complicated patterns. Note, however, that this applies only to data characters. 
...
// ======== Determine compiler
{$IFDEF DELPHI1} Sorry, TRegExpr is for 32-bits Delphi only. Delphi 1 is not supported (and whos really care today?!). {$ENDIF}

{$IFNDEF FPC} // the option is not supported in FreePascal
 {$DEFINE RaiseOnRealExceptionAddr} // exceptions will point to appropriate source line, not to Error procedure
{$ENDIF}
{$IFNDEF UniCode_RegExpr} // the option applicable only for non-UniCode mode
...
{$IFDEF COMPILER3_UP} {$DEFINE UseAsserts} {$ENDIF}
{$IFDEF FPC} {$DEFINE UseAsserts} {$ENDIF}

uses
  {$IFDEF SplitMethod_RegExpr}
  Classes,  // TStrings in Split method
  {$ENDIF}
  SysUtils; // Exception

type
 {$IFDEF UniCode_RegExpr}
...
const
  ...
  RegExprLineSeparators : RegExprString =// default value for LineSeparators
   #$d#$a{$IFDEF UniCode_RegExpr}+#$b#$c#$2028#$2029#$85{$ENDIF}; //###0.947
...
{$ENDIF}
 ...
 TRegExprReplaceFunction = function (const aRegExpr : TRegExpr): RegExprString of object;
 ...
 TRegExpr = class
   ...
   protected
    ...
    //Keep the SubExpr Name parse from the Expression, now we can use SubExpr Name get the SubExpr String. 
    //added by riceball
    //the Define: "(....):SubExprName:"
    FSubExprNames: array [0 .. NSUBEXP - 1] of RegExprString;
    {$ENDIF}
    {$IFDEF CapturingGroup_RegExpr}
    FCapturingGroupCount: Integer; // the real SubExprCount is FCapturingGroupCount-1, do not include the skiped group.
    FCapturingGroup: array [0 .. NSUBEXP - 1] of byte;  //the  index of the startp
    {$ENDIF}
    ...
    regAnchor : REChar; // is the match anchored (at beginning-of-line only)?
    regMustStr : PRegExprChar; // string (pointer into program) that match must include, or nil
    regMustStrLen : integer; // length of regMustStr string
    // Regstart and regAnchor permit very fast decisions on suitable starting points
    // for a match, cutting down the work a lot.  regMustStr permits fast rejection
    // of lines that cannot possibly match.  The regMustStr tests are costly enough
    // that regcomp() supplies a regMustStr only if the r.e. contains something
    // potentially expensive (at present, the only such thing detected is * or +
    // at the start of the r.e., which can involve a lot of backup).  regMustStrLen is
    ...
    regnpar : integer; // count. the brackets(SubExpr) count?
    regdummy : char;
    regcode : PRegExprChar;   // Code-emit pointer(compiled P-code pointer: program or regdummy[only for calc program(p-code) size]); @regdummy = don't.
    ...
    FInputStringPtr : PRegExprChar; // input string
    ...
    procedure ClearMatchedResults;  //##1.000
    ...
    {$IFDEF PCodeDump_RegExpr}
    function DumpOp (op : REChar) : RegExprString;
    {$ENDIF}

    function GetSubExprMatchCount : integer;
    function iGetMatchPos (const Idx : integer) : integer;
    function iGetMatchLen (const Idx : integer) : integer;
    function iGetMatch (const Idx : integer) : RegExprString;
    ...
    {$IFDEF SubExprName_RegExpr}
    //added by riceball
    function GetMatchStrPos(const aSubExprName: RegExprString): integer;
    function GetMatchStrLen(const aSubExprName: RegExprString): integer;
    function GetMatchStr(const aSubExprName: RegExprString): RegExprString;
    function GetSubExprName(const index: integer): RegExprString;
    {$ENDIF}
   ...
   public
    ...
    {$IFDEF SubExprName_RegExpr}
    // added by riceball
    // return the SubExpr Index, if not found then return -1
    // See Also Match, MatchPos, MatchLen
    ...
    function Exec (const AInputString : RegExprString) : boolean; {$IFDEF SUPPORTS_OVERLOAD} overload;
    ...
    function ExecPos (AOffset: integer {$IFDEF SUPPORTS_DEFAULTPARAMS}= 1{$ENDIF}) : boolean;
    ...
    procedure SetSubjectStr(const Value: RegExprString); //##1.000
    procedure SetSubjectPChar(const Value: PRegExprChar);  //##1.000
    //SetSubjectStr does not assign the Value, just a pointer to the string's beginning.
    //for get more performance, so you must not free the Value when matching.
    ...
    function Replace (const AInputStr : RegExprString;
      ...
      AUseSubstitution : boolean{$IFDEF SUPPORTS_DEFAULTPARAMS}= False{$ENDIF}) //###0.946
     : RegExprString; {$IFDEF SUPPORTS_OVERLOAD} overload;
    function Replace (const AInputStr : RegExprString;
...
function ReplaceRegExpr (const ARegExpr, AInputStr, AReplaceStr : RegExprString;
      AUseSubstitution : boolean{$IFDEF SUPPORTS_DEFAULTPARAMS}= False{$ENDIF}) : RegExprString; //###0.947
// Returns AInputStr with r.e. occurencies replaced by AReplaceStr
// If AUseSubstitution is true, then AReplaceStr will be used
// as template for Substitution methods.
// For example:
...
function RegExprSubExpressions (const ARegExpr : string;
 ASubExprs : TStrings; AExtendedSyntax : boolean{$IFDEF SUPPORTS_DEFAULTPARAMS}= False{$ENDIF}) : integer;
...
const
 ...
 TRegExprVersionMinor : integer = 980;
...
function ReplaceRegExpr (const ARegExpr, AInputStr, AReplaceStr : RegExprString;
      AUseSubstitution : boolean{$IFDEF SUPPORTS_DEFAULTPARAMS}= False{$ENDIF}) : RegExprString;
 begin
...
// name            opcode    opnd? meaning
 ...
 OPEN        = TREOp (39); // -    Mark this point in input as start of \n, the first SubExpr
                           //      OPEN + 1 is \1, etc.
 //NSubExp .... See L: 3334
 CLOSE       = TREOp (ord (OPEN) + NSUBEXP); //the END NSubExp.
                           // -    Analogous to OPEN.
...
destructor TRegExpr.Destroy;
 begin
  ...
  //if FInputStringPtr <> nil
   //then FreeMem (FInputStringPtr);
...
begin
  if Assigned (FInputStringPtr) then 
  begin
   {$IFDEF CapturingGroup_RegExpr}
    Result := FCapturingGroupCount - 1;
   {$ELSE}
     Result := NSUBEXP - 1;
...
{$IFDEF SubExprName_RegExpr}
function TRegExpr.GetMatchStrPos(const aSubExprName: RegExprString): integer;
var
  idx: integer;
begin
  idx := GetSubExprIndexByName(aSubExprName);
  Result := iGetMatchPos(idx);
end;

function TRegExpr.GetMatchStrLen(const aSubExprName: RegExprString): integer;
var
  idx: integer;
begin
  idx := GetSubExprIndexByName(aSubExprName);
  Result := iGetMatchLen(idx);
end;

function TRegExpr.GetMatchStr(const aSubExprName: RegExprString): RegExprString;
var
  idx: integer;
begin
  idx := GetSubExprIndexByName(aSubExprName);
  Result := iGetMatch(idx);
end;

{$ENDIF SubExprName_RegExpr}

function TRegExpr.iGetMatchPos (const Idx : integer) : integer;
 begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned (FInputStringPtr)
     and Assigned (startp [Idx]) and Assigned (endp [Idx]) then begin
     Result := (startp [Idx] - FInputStringPtr) + 1;
    end
   else Result := -1;
 end; { of function TRegExpr.iGetMatchPos
--------------------------------------------------------------}

function TRegExpr.iGetMatchLen (const Idx : integer) : integer;
 begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned (FInputStringPtr)
     and Assigned (startp [Idx]) and Assigned (endp [Idx]) then begin
 ...
 end; { of function TRegExpr.iGetMatchLen
--------------------------------------------------------------}

function TRegExpr.iGetMatch (const Idx : integer) : RegExprString;
 begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned (FInputStringPtr)
     and Assigned (startp [Idx]) and Assigned (endp [Idx])
   //then Result := copy (FInputStringPtr, MatchPos [Idx], MatchLen [Idx]) //###0.929
   then SetString (Result, startp [idx], endp [idx] - startp [idx])
   else Result := '';
 end; { of function TRegExpr.iGetMatch
--------------------------------------------------------------}

function TRegExpr.GetMatchPos (Idx : integer) : integer;
 begin
{$IFDEF CapturingGroup_RegExpr}
  Idx := FCapturingGroup[Idx];
{$ENDIF}
...
// Flags to be passed up and down.
 ...
 SkipGroup=   08; // skip sub-expcession group. (?:...)
 WORST    =   0;  // Worst case.
...
//   end;
  ...
  if PREOp (regnext (scan))^ = EEND then begin // Only one top-level choice.
    ...
    // literal string that must appear and make it the regMustStr.  Resolve
...
// follows makes it hard to avoid.
 ...
 begin
  ...
  if (paren <> 0) then begin // sub-expression
      ...
      parno := regnpar;
      {$IFDEF CapturingGroup_RegExpr}
      if flagp and SkipGroup <> SkipGroup then
      begin
        FCapturingGroup[FCapturingGroupCount] := parno;
        //OutputDebugString('FCapturingGroup['+IntToStr(FCapturingGroupCount)+']='+IntToStr(parno));
        Inc(FCapturingGroupCount);
      end;
      {$ENDIF}
      inc (regnpar);
  ...
  if (paren <> 0) //and (flagp and SkipGroup <> SkipGroup)
   then ender := EmitNode (TREOp (ord (CLOSE) + parno))
   else //if paren = 0 then 
     ender := EmitNode (EEND)
   //else ender := nil
   ;

  //set the next-pointer at the end of a node chain
  //if assigned(ender) then
  Tail (ret, ender);
...
//          flagp := flagp or HASWIDTH or SIMPLE;
    ...
    '(': begin
        ...
        if regparse^ = '?' then begin
          ...
          case (regparse + 1)^ of
           {.$IFDEF CapturingGroup_RegExpr} //to keep the compatible with the Perl RegExpr.
...
//             EXIT;
         ...
         //else
         if not assigned(ret) then begin
    ...
    EscChar: begin
      ...
      if ((fCompModifiers and MaskModX) <> 0) and // check for eXtended syntax
          ((regparse^ = '#')
           or ({$IFDEF UniCode_RegExpr}StrScan (XIgnoredChars, regparse^) <> nil //###0.947
         ...
         if regparse^ = '#' then begin // Skip eXtended comment
            ...
            while {$IFDEF UniCode_RegExpr}StrScan (XIgnoredChars, regparse^) <> nil //###0.947
         ...
         while (len > 0)
          and (((fCompModifiers and MaskModX) = 0) or (regparse^ <> '#')) do begin
           if ((fCompModifiers and MaskModX) = 0) or not ( //###0.941
              {$IFDEF UniCode_RegExpr}StrScan (XIgnoredChars, regparse^) <> nil //###0.947
              {$ELSE}regparse^ in XIgnoredChars{$ENDIF} )
            then EmitC (regparse^);
           inc (regparse);
           dec (len);
     ...
     case scan^ of
         ...
         ANYML: begin //###0.941
            if (regInput^ = #0)
             ...
             or {$IFNDEF UniCode_RegExpr} (regInput^ in fLineSeparatorsSet)
...
function TRegExpr.ExecPos (AOffset: integer {$IFDEF SUPPORTS_DEFAULTPARAMS}= 1{$ENDIF}) : boolean;
 begin
  Result := ExecPrim (AOffset);
 end; { of function TRegExpr.ExecPos
--------------------------------------------------------------}

procedure TRegExpr.ClearMatchedResults;
// Clears matchs array
var i : integer;
begin
 for i := 0 to NSUBEXP - 1 do begin
   if startp [i] = nil then break;
   startp [i] := nil;
   endp [i] := nil;
   {$IFDEF SubExprName_RegExpr}
   FSubExprNames[i] := '';
  ...
  {$IFDEF CapturingGroup_RegExpr}
  //FCapturingGroupCount := 0;
  {$ENDIF}
end; { of procedure ClearMatchedResults;
--------------------------------------------------------------}

function TRegExpr.ExecPrim (AOffset: integer) : boolean;
 {procedure ClearMatchedResults;
  // Clears matchs array
  var i : integer;
  begin
   for i := 0 to NSUBEXP - 1 do begin
...
..............................................................}
 ...
 begin
  ...
  //if (programm = nil) or not Assigned (FInputStringPtr) then exit;
  if not IsProgrammOk //###0.929  //??fExprIsCompiled 
   then EXIT;

  // Check InputString presence
  if not Assigned (FInputStringPtr) then begin
    Error (reeNoInpitStringSpecified);
    EXIT;
   end; //}

  //InputLen := length (FInputStringPtr); //speedup via Riceball, this will be very slow on a large PChar.
  ...
  if AOffset > (FInputStringLen + 1) // for matching empty string after last char.
   then EXIT;
  ...
  if regMustStr <> nil then begin
    s := StartPtr;
    REPEAT
     s := StrScan (s, regMustStr [0]);
     if s <> nil then begin
       if StrLComp (s, regMustStr, regMustStrLen) = 0
        then BREAK; // Found it.
       inc (s);
      end;
    UNTIL s = nil;
  ...
  if regstart <> #0 then // We know what char it must start with.
    REPEAT
     ...
     if s <> nil then begin
       ...
       if Result
        then EXIT
        else ClearMatchedResults; //###0.949
       inc (s);
      end;
    UNTIL s = nil
...
//  if MatchLen [0] = 0
  Offset := endp [0] - FInputStringPtr + 1; //###0.929
...
function TRegExpr.GetInputString : RegExprString;
 begin
  ...
  Result := FInputStringPtr;
 end; { of function TRegExpr.GetInputString
--------------------------------------------------------------}

procedure TRegExpr.SetSubjectPChar(const Value: PRegExprChar);
begin
  // clear Match* - before next Exec* call it's undefined
  ClearMatchedResults;

  FInputStringPtr := Value;
  FInputStringLen := Length(Value);
end;

procedure TRegExpr.SetSubjectStr(const Value: RegExprString);
begin
  // clear Match* - before next Exec* call it's undefined
  ClearMatchedResults;

  FInputStringPtr := PRegExprChar(Value);
  FInputStringLen := Length(Value);
end;
...
procedure TRegExpr.SetInputString (const AInputString : RegExprString);
 ...
 begin
  ...
  FInputStringPtr := PRegExprChar(FInputString);
  FInputStringLen := Length(AInputString);

  (*// need reallocation of input string buffer ?
  Len := length (AInputString);
  if Assigned (FInputStringPtr) and (Length (FInputStringPtr) <> Len) then begin
    FreeMem (FInputStringPtr);
    FInputStringPtr := nil;
   end;
  // buffer [re]allocation
  if not Assigned (FInputStringPtr)
   then GetMem (FInputStringPtr, (Len + 1) * SizeOf (REChar));

  // copy input string into buffer
  {$IFDEF UniCode_RegExpr}
  StrPCopy (FInputStringPtr, Copy (AInputString, 1, Len)); //###0.927
  {$ELSE}
  StrLCopy (FInputStringPtr, PRegExprChar (AInputString), Len);
  {$ENDIF}*)

  {
  FInputStringPtr : string;
  fInputStart, fInputEnd : PRegExprChar;

  SetInputString:
  FInputStringPtr := AInputString;
  UniqueString (FInputStringPtr);
...
// completely rewritten in 0.929
 ...
 function ParseVarName (var APtr : PRegExprChar) : integer;
  ...
  var
   ...
   s: ShortString;
  begin
   //added and modified by riceball
   // we suppose that no valid digits found or no right curly brace
   Result := -1;
   p := APtr;
   s := '';
   {$IFDEF SubExprName_RegExpr}
   vNamed := p^ = '[';
   {$ENDIF}
   Delimited := (p < TemplateEnd) and ((p^ = '{') {$IFDEF SubExprName_RegExpr}or vNamed{$ENDIF});
   if Delimited then inc (p); // skip left curly brace
   ...
   then begin
     ...
     while (p < TemplateEnd) and
      ...
      if (Result >= NSUBEXP) or not Assigned (startp [Result]) or not Assigned (endp [Result]) then Result := -1;
   end;
   if Delimited then
    if (p < TemplateEnd) and ((p^ = '}') {$IFDEF SubExprName_RegExpr}or (p^ = ']'){$ENDIF}) then
    begin
      {$IFDEF SubExprName_RegExpr}
      if vNamed then
        Result := GetSubExprIndexByName(s);
      {$ENDIF}
      ...
      p := APtr//};
   //if p = APtr then Result := -1 // no valid digits found or no right curly brace
     
   if (Result >= 0) then APtr := p;
  end;
 begin
...
{$ENDIF}
 begin
  ...
  if regMustStr <> nil
   then Result := Result + 'must have ' + regMustStr;
  {$IFDEF UseFirstCharSet} //###0.929
...
{$IFDEF RaiseOnRealExceptionAddr}
{$OPTIMIZATION ON}
...
(*
  PCode persistence:
   ...
   regstart // -> programm
   regAnchor // -> programm
   regMustStr, regMustStrLen // -> programm
   fExprIsCompiled
*)
...
{$IFDEF SubExprName_RegExpr}
Function TRegExpr.GetSubExprIndexByName(const aSubExprName: RegExprString) : Integer;
var
  i: integer;
begin
  if Assigned (FInputStringPtr) then 
    ...
    Begin
      if (FSubExprNames[i] <> '') and (FSubExprNames[i] = aSubExprName) then
      begin
        Result := i;
        Exit;
      end;
...
function TRegExpr.GetSubExprName(const index: integer): RegExprString;
begin
  If (index >= 0) and (index < NSUBEXP) Then
  Begin
    Result := FSubExprNames[index];
  End
  Else 
    Result := '';
end;
