
{
  ...
  License:
    ...
    * The Original Code is $RCSfile: uMePathUtils.pas,v $.
    * The Initial Developers of the Original Code are Riceball LEE.
    * Portions created by Juancarlo Anez are Copyright (C) 2001-2003
...
begin
  ...
  uPathUtils.CopyFile(Src, Dst);
  uPathUtils.DeleteFile(Src);
end;
...
begin
   ...
   Handle := FileOpen(s, fmOpenWrite or fmShareDenyNone);
   if Handle < 0 then
   begin
     Handle := FileCreate(s);
     FileClose(Handle);
   end;
   FileSetDate(s, DateTimeToFileDate(When))
end;
